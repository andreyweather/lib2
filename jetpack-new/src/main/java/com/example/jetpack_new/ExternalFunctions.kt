...
internal fun NotificationCompat.Builder.setLargeIcon(uri: Uri?): NotificationCompat.Builder {
    setLargeIcon(EnkodPushLibrary.getBitmapFromUrl(uri.toString()))
    return this
}

internal fun NotificationCompat.Builder.setStyle(message: RemoteMessage): NotificationCompat.Builder {
        val imgName = message.notification?.imageUrl
        val body = message.notification?.body
        if(imgName != null){
            setStyle(
                NotificationCompat.BigPictureStyle()
                    .bigPicture(EnkodPushLibrary.getBitmapFromUrl(message.notification?.imageUrl.toString()))
                    .setSummaryText(body)
            )
        }else{
            setStyle(
